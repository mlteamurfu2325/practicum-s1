...
def test_element_appearance(page):
    ...
    page.get_by_test_id("stSelectbox").locator("div").filter(
        has_text="С Вашего устройства"
    ).nth(2).click()
    page.get_by_text("С YouTube").click()
    page.get_by_placeholder("Ссылка формата https://www.").click()
    page.get_by_placeholder("Ссылка формата https://www.").fill(
        "https://www.youtube.com/watch?v=m9NhYYg8jBo"
    )
